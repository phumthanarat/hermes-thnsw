...
<xsl:template match="/partnerships">
	...
	<xsl:if test="count(partnership)>0">
	    ...
	    <input type="text" id="pshipSearch" placeholder="Search by partnership ID..." onkeyup="filterPships()"
	    ...
	    <table border="0" cellpadding="2" cellspacing="2" width="100%" id="pshipTable">
	      ...
	      <xsl:for-each select="partnership">
	        <tr>
	          <xsl:attribute name="data-s"><xsl:value-of select="translate(concat(./partnership_id),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')" /></xsl:attribute>
