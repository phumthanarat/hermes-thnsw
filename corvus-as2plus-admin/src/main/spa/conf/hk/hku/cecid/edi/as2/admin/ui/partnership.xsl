...
<xsl:template match="/partnerships">
	...
	<xsl:if test="count(partnership)>0">
	    ...
	    <input type="text" id="pshipSearch" placeholder="Search by AS2 From or AS2 To..." onkeyup="filterPships()"
	    ...
	    <table border="0" cellpadding="2" cellspacing="2" width="100%" id="pshipTable">
	      ...
	      <xsl:for-each select="partnership">
	        <tr>
	          <xsl:attribute name="data-s"><xsl:value-of select="translate(concat(./as2_from,' ',./as2_to,' ',./partnership_id),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')" /></xsl:attribute>
	          ...
	          <td><xsl:value-of select="./as2_from" /></td>
	          <td><xsl:value-of select="./as2_to" /></td>
	          <td style="font-size:12px; color:var(--text-soft);"><xsl:value-of select="./partnership_id" /></td>
	          <td>
